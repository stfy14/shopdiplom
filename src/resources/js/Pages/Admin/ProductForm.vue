...
// Текущая категория и её характеристики
const selectedCategory = ref(
    props.categories.find(c => c.id === props.product?.category_id) ?? null
)

// Предзаполняем характеристики при редактировании
    ...
    <div class="min-h-screen bg-gray-50">
        <nav class="bg-white shadow-sm mb-6">
            <div class="max-w-3xl mx-auto px-6 py-4 flex items-center gap-4">
                <Link href="/admin" class="w-10 h-10 border rounded-xl flex items-center justify-center hover:bg-gray-50">
                    ←
                </Link>
                <h1 class="font-bold text-xl">{{ isEdit ? 'Редактирование товара' : 'Новый товар' }}</h1>
            </div>
        </nav>
