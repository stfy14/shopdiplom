...
<template>
    <div>
        ...
        <div class="flex items-center gap-4 mb-6">
            <div class="w-10 h-10 bg-white border border-gray-200 rounded-xl flex items-center justify-center text-gray-600 shadow-sm">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z"/>
                </svg>
            </div>
            <div>
                <h1 class="text-2xl font-black text-gray-900">Запросы на звонок</h1>
                <p class="text-sm text-gray-400 font-medium mt-0.5">Заявки с формы на главной странице</p>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-3">
            ...
            <!-- Пусто -->
            <div v-if="filteredCallbacks.length === 0" class="text-center py-16 bg-white rounded-3xl border border-gray-100 shadow-sm">
                <div class="w-14 h-14 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    <svg class="w-7 h-7 text-gray-300" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z"/>
                    </svg>
                </div>
                <div class="font-bold text-gray-500 text-base">{{ tab === 'new' ? 'Новых заявок нет' : 'Обработанных заявок нет' }}</div>
                <div class="text-sm text-gray-400 mt-1">
                    {{ tab === 'new' ? 'Новые заявки появятся здесь' : 'Завершённые заявки появятся здесь' }}
                </div>
            </div>
            ...
            <div
                ...
                :key="cb.id"
                class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 transition hover:shadow-md
                       flex flex-col gap-3
                       md:grid md:grid-cols-12 md:gap-4 md:items-center md:flex-none"
            >
                <!-- Дата -->
                <div class="md:col-span-2">
                    <div class="text-xs text-gray-400 font-mono leading-relaxed">{{ formatDate(cb.created_at) }}</div>
                </div>

                <!-- Имя -->
                <div class="md:col-span-3 flex items-center justify-between md:block">
                    <div class="font-bold text-gray-900">{{ cb.name }}</div>
                    <!-- Телефон только на мобилке -->
                    <a :href="`tel:${cb.phone.replace(/\D/g, '')}`" class="md:hidden font-black text-blue-600 text-sm hover:text-blue-800 transition">
                        {{ cb.phone }}
                    </a>
                </div>
                ...
                <div class="hidden md:block md:col-span-3">
                    <a :href="`tel:${cb.phone.replace(/\D/g, '')}`" class="font-black text-blue-600 hover:text-blue-800 transition flex items-center gap-2 group">
                        <div class="w-7 h-7 bg-blue-50 group-hover:bg-blue-600 text-blue-600 group-hover:text-white rounded-lg flex items-center justify-center transition flex-shrink-0">
                ...
                <!-- Комментарий -->
                <div class="md:col-span-2">
                    <div v-if="cb.comment" class="text-sm text-gray-500 font-medium italic line-clamp-2 bg-gray-50 px-2.5 py-1.5 rounded-lg border border-gray-100">
                        «{{ cb.comment }}»
                    </div>
                    <div v-else class="flex items-center gap-1.5 text-xs text-gray-300 font-medium">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M18 12H6"/></svg>
                        Без комментария
                    </div>
                </div>
                ...
                <div class="md:col-span-2 flex items-center justify-end gap-2">
                    <!-- Кнопка "Готово" только для новых -->
                    <button
                        v-if="cb.status === 'new'"
                    ...
                    <!-- Значок "Обработано" вместо кнопки -->
                    <div
                        v-else
                        class="flex items-center gap-1.5 px-3 py-2 bg-gray-50 text-gray-400 rounded-xl text-xs font-bold border border-gray-100"
                    >
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                    ...
                    <button
                        @click="destroy(cb.id)"
                        class="w-9 h-9 bg-gray-50 hover:bg-red-50 text-gray-400 hover:text-red-500 rounded-xl flex items-center justify-center transition shadow-sm border border-gray-100"
